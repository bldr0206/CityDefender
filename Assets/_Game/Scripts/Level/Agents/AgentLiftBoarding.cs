...
/// Прерывания и координацию с боем/следованием делает <see cref="Agent"/>.
/// </summary>
public sealed class AgentLiftBoarding
{
    enum Phase
    {
        None,
    ...
    public AgentLiftBoarding(Agent owner)
    {
        _owner = owner;
        _loco = owner.Loco;
    ...
    /// <summary> Начать подход к лифту. Прерывание других поведений уже сделал <see cref="Agent"/>. </summary>
    public void Begin(Lift lift, Vector3 approachWorldHint, Transform seatPoint, float seatBoardDuration)
    {
        ...
        {
            Debug.LogWarning($"{nameof(Agent)}: подход к лифту — точка не на NavMesh.", _owner);
            return;
        }
        ...
        {
            Debug.LogWarning($"{nameof(Agent)}: подход к лифту — нет полного NavMesh-пути.", _owner);
            return;
        }
